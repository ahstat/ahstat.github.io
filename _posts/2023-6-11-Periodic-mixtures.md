...
$$
\begin{align*}
  Z_t f_{\sigma}(z) =& 2 t \sum_{k=1}^{\lfloor \frac{1}{2t} \rfloor} \cos \left( 2 k \pi z \right).
\end{align*}
$$
...
For $$\lambda > 0$$, we consider $$x \in [-\lambda/2, \lambda/2)$$, or equivalently $$z \in [-1/2, 1/2)$$.
The closed-form expression for the Sinc type verifies, from the previous proof:

$$
\begin{align*}
S_{\lambda}f_{\sigma}(x) - \frac{1}{\lambda} =& \frac{2}{\lambda} \sum_{k=1}^{\lfloor \frac{\lambda}{2\sigma} \rfloor} \cos \left( 2 \pi k x / \lambda \right) \\
=& \frac{1}{\sigma} 2 t \sum_{k=1}^{\lfloor \frac{1}{2t} \rfloor} \cos \left( 2 k \pi z \right).
\end{align*}
$$

Using the normalization: $$N(\lambda, \sigma) = \sigma$$, we obtain:
...
<!--
Using the normalization: $$N(\lambda, \sigma) = \lambda$$, we obtain:

$$
\begin{align*}
Z_t f_{\sigma}(z) =& 2 \sum_{k=1}^{\lfloor \frac{1}{2t} \rfloor} \cos \left( 2 k \pi z \right).
\end{align*}
$$
-->
...
Restricting on the single period $$z \in [-1/2, 1/2)$$, we observe in the following video:
- (left) the mapping $$z \mapsto Z_t f_{\sigma}(z)$$ over time, with marked points for $$z=-1/2$$ (blue), $$z=\pm 1/4$$ (green), $$z= \pm 1/6$$ (yellow) and $$z=0$$ (gray); we observe the cosinus function for $$t=1/2$$ and the null function for larger values of $$t$$.
- (center) the mapping $$t \mapsto Z_t f_{\sigma}(z)$$ for the $$6$$ previous marked points over time,
- (right) the gray borders correspond to the points were $$Z_t f_{\sigma}(z) = 0$$. The inner gray region corresponds to $$Z_t f_{\sigma}(z) \geq 0$$, while the outside verifies $$Z_t f_{\sigma}(z) \leq 0$$. The zone on the right is when the function is null.

<center><video controls autoplay=1 loop=1 src="../images/2023-6-11-Periodic-mixtures/plot3/sinc/function_of_time.mp4"></video></center>
...
**Rectangular.** Since $$S_{\lambda}g_{\sigma}(x) = 0$$ for all regular $$x$$, the parametric curve is degenerate (it traces a segment on the horizontal axis), so $$\mathcal{A} = 0$$. The Fourier sum $$\sum k^2 \text{sinc}^2(kt) = \frac{1}{\pi^2 t^2} \sum \sin^2(\pi k t)$$ diverges, so the Fourier area formula does not apply to this type.

**Linear.** Using $$k^2 \text{sinc}^4(kt) = \frac{\sin^4(\pi k t)}{\pi^4 t^4 k^2}$$ and the decomposition $$\sin^4(\pi k t) = \frac{3}{8} - \frac{1}{2}\cos(2\pi k t) + \frac{1}{8}\cos(4\pi k t)$$, together with the identity $$\sum_{k=1}^{\infty} \frac{\cos(2\pi k \alpha)}{k^2} = \pi^2 B_2(\lbrace \alpha \rbrace)$$ where $$B_2(x) = x^2-x+\frac{1}{6}$$, we obtain:

$$\sigma^3 \mathcal{A} = \frac{1}{t}\left[\frac{1}{2} - 4 B_2(\lbrace t \rbrace) + B_2(\lbrace 2t \rbrace)\right].$$

For $$\lbrace t \rbrace \in [0, \frac{1}{2})$$: $$\lbrace 2t \rbrace = 2\lbrace t \rbrace$$, and the bracket simplifies to $$\frac{1}{2} - 4(\lbrace t \rbrace^2 - \lbrace t \rbrace + \frac{1}{6}) + (4\lbrace t \rbrace^2 - 2\lbrace t \rbrace + \frac{1}{6}) = 2\lbrace t \rbrace$$.

For $$\lbrace t \rbrace \in [\frac{1}{2}, 1)$$: $$\lbrace 2t \rbrace = 2\lbrace t \rbrace - 1$$, and the bracket simplifies to $$2(1-\lbrace t \rbrace)$$.

In both cases: $$\sigma^3 \mathcal{A} = \frac{2\min(\lbrace t \rbrace, 1-\lbrace t \rbrace)}{t}$$, which is the tent function divided by $$t$$, $$1$$-periodic, vanishing at integers and peaking at $$1/t$$ at half-integers.

**Exponential.** From the identity $$\sum_{k=1}^{\infty} \frac{k^2}{(1+b^2 k^2)^2} = \frac{\pi}{4b^4}\left(\frac{b}{\tanh(\pi/b)} - \frac{\pi}{\sinh^2(\pi/b)}\right)$$ with $$b = \pi t$$, we derive after simplification:

$$\sigma^3 \mathcal{A} = 2\left(\coth \frac{1}{t} - \frac{1}{t \sinh^2 \frac{1}{t}}\right).$$

The series expansion $$\coth u - u\,\text{cosech}^2 u = \frac{2u}{3} - \frac{4u^3}{45} + \cdots$$ with $$u = 1/t$$ gives $$\sigma^3 \mathcal{A} \sim \frac{4}{3t}$$ for $$t$$ large.

**Polynomial.** Using $$\mathcal{F}f_\sigma(k/\lambda) = e^{-2kt}$$ and the generating function $$\sum_{k=1}^{\infty} k^2 q^k = q(1+q)/(1-q)^3$$ with $$q = e^{-4t}$$:

$$\sum_{k=1}^{\infty} k^2 e^{-4kt} = \frac{e^{-4t}(1+e^{-4t})}{(1-e^{-4t})^3} = \frac{\cosh(2t)}{4\sinh^3(2t)},$$

so $$\sigma^3 \mathcal{A} = \frac{2\pi^2 t^3 \cosh(2t)}{\sinh^3(2t)}$$. For $$t$$ large: $$\cosh(v)/\sinh^3(v) \sim 4 e^{-2v}$$, giving $$\sigma^3 \mathcal{A} \sim 8\pi^2 t^3 e^{-4t}$$.

**Gaussian.** Using $$\mathcal{F}f_\sigma(k/\lambda) = e^{-\pi k^2 t^2}$$, the sum $$\sum k^2 e^{-2\pi k^2 t^2}$$ is related to the second derivative of the Jacobi theta function: $$\sigma^3 \mathcal{A} = -\pi^2 t^3 \vartheta_3''(0, e^{-2\pi t^2})$$. For $$t$$ large, the dominant $$k=1$$ term gives $$\sigma^3 \mathcal{A} \sim 8\pi^2 t^3 e^{-2\pi t^2}$$.

**Sinc.** From $$\mathcal{F}f_\sigma(k/\lambda) = \mathbf{1}_{k \leq 1/(2t)}$$, only $$N = \lfloor 1/(2t) \rfloor$$ terms contribute, giving $$\sigma^3 \mathcal{A} = \frac{4\pi^2 t^3 N(N+1)(2N+1)}{3}$$. For $$t > 1/2$$: $$N = 0$$ and $$\mathcal{A} = 0$$. The area jumps discontinuously at each $$t = 1/(2n)$$, with jump magnitude $$\pi^2/n$$.

**Sincsquare.** From $$\mathcal{F}f_\sigma(k/\lambda) = (1-kt)\mathbf{1}_{k \leq 1/t}$$: only $$N = \lfloor 1/t \rfloor$$ terms contribute. For $$t \geq 1$$: $$N = 0$$ and $$\mathcal{A} = 0$$ exactly. The onset is continuous: for $$t \lesssim 1$$, only $$k=1$$ contributes, giving $$\sigma^3 \mathcal{A} \sim 8\pi^2(1-t)^2/t$$.

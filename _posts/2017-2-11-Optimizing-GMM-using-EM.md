...
*Gaussian Mixture Models* (GMM) is a parametric clustering method which models data as a finite mixture of Gaussians.
...
In addition, we assume that $$x_i$$ is labeled with a certain $$k \in \lbrace 1, \ldots, K$$,
where $$K$$ is a fixed integer.
The underlying random variable to model the label is noted $$Z_i$$, and the probability
to be labeled as $$k$$ is written as follows:
...
Using the law of total probability, we reveal the hidden variable (where $$x \in \mathbb{R}^d$$):
...
- Each conditional variable $(X_i | Z_i = k)$ follows a Gaussian distribution with mean and variance $m_k, \Sigma_k$.

Unknown parameters of the model are grouped together into:

$$\theta := ((\pi_k)_{k \in \lbrace 1, \ldots, K}, (m_k)_{k \in \lbrace 1, \ldots, K}, (\Sigma_k)__{k \in \lbrace 1, \ldots, K}).$$

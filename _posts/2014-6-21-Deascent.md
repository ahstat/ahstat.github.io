...
Let $$ꞙ$$ a function on $$\mathbb{R}^{+}$$ such that $$ꞙ(0) = 0$$.
...
We define the force of the action as proportional to $$ꞙ$$ of the distance between $$x_i$$ and $$x_j$$ i.e.  $$ꞙ(\| x_j - x_i \|)$$.

We also multiply by $$\kappa_j$$ the densitype of $$x_j$$.

Finally, the action of $$x_j$$ on $$x_i$$ is given by:

$$\kappa_j \times ꞙ(|| x_j - x_i ||) \times (x_j - x_i) / || x_j - x_i ||.$$

The global action of all the points is given by:

$$\frac{1}{n} \sum_{j=1}^{n} \kappa_j \times ꞙ(|| x_j - x_i ||) \times (x_j - x_i) / || x_j - x_i ||.$$



Note that in this formula we let the action of $$i$$ on itself at $$0$$. This is why $$ꞙ(0)$$ must be $$0$$.
Better, we would like $$x \times ꞙ(|| x ||) / || x || --> 0$$ when $$|| x || --> 0$$, so
$$ꞙ(|| x ||) --> 0$$ when $$|| x || --> 0$$

The global action with $$\iota_i$$ is given by:

$$F_i := \iota_i \times \frac{1}{n} \sum_{j=1}^{n} \kappa_j \times ꞙ(|| x_j - x_i ||) \times (x_j - x_i) / || x_j - x_i ||.$$
